...
SELECT cust_balance FROM customer
...
SELECT cust_name,cust_balance FROM customer
WHERE cust_balance = (SELECT Min (cust_balance)FROM customer );
...
SELECT rep_id, SUM(cust_balance) AS Total_balance FROM customer
GROUP BY rep_id
ORDER BY rep_id
